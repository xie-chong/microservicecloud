...
**优化**   
若是每个服务提供的方法上都搞一个@HystrixCommand(fallbackMethod = "processHystrix_Get")以及相应的处理方法，那么类方法就会急剧膨胀，注解也会很多很繁琐。这样不好。以上是在服务端设置Hystrix的注解，现在为了避免啰嗦的问题，解决方法如下：（转移到客户端解决--服务降级）
...
server:
  port: 9001
```
4. 新建主启动类,添加注解@**EnableHystrixDashboard**
```
@SpringBootApplication
...
```
5. 所有服务提供者微服务(Provider 8001/8002/8003)都需要监控依赖配置(需要使用聚合服务才能统一监控集群服务，本次操作仅展示单体应用)
```
 <!-- actuator监控信息完善 -->
   <dependency>
     <groupId>org.springframework.boot</groupId>
...
```
6. 启动微服务microservicecloud-consumer-hystrix-dashboard，访问```http://localhost:9001/hystrix```
7. 启动3个eureka集群
8. 启动服务提供者microservicecloud-provider-dept-hystrix-8001
```
http://localhost:8001/dept/get/1
// ping 监控的仪表盘 json数据
http://localhost:8001/hystrix.stream
```
9. 确定相关的微服务工程都已经启动（7001/7002/7003->provider-hystrix-8001->hystrix-dashboard）
10. 监控测试
